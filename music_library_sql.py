...
if __name__ == "__main__":
    ...
    spark = (SparkSession.builder.appName("MusicLibaryExploring").getOrCreate())

    ##Assign file path to a variable
    music_data_file_path = sys.argv[1]

    ## Load the csv into a dataframe
    lib_df = spark.read.format("csv").option("header", "true").option("inferSchema", "true").load(music_data_file_path)
    lib_df.show()
